...
/**
 * What the main pane shows: the course-level materials overview, a specific
 * chapter's content, or the curriculum list (reachable via the sidebar).
 */
  ...
  useEffect(() => {
    fetchCourse(courseId)
      .then(({ course, sections, chapters }) => {
        ...
        // Deep-link: honour ?chapter=<id> when it points at a real chapter,
        // otherwise open the first chapter, then materials, then curriculum.
        const requested =
          requestedChapter && chapters.some((c) => c.id === requestedChapter)
            ? requestedChapter
            : null;
        if (requested) setSelection({ kind: "chapter", id: requested });
        else if (chapters.length > 0) setSelection({ kind: "chapter", id: chapters[0].id });
  ...
  return (
    <>
      ...
      <aside className="sticky top-20 hidden h-fit rounded-2xl border border-border bg-card p-3 shadow-sm lg:block">
        <Link
          href={`/courses/${courseId}`}
          className="mb-1 flex items-center gap-1 px-2 pt-1 text-xs font-medium text-primary hover:underline"
        >
          ← Course overview
        </Link>
